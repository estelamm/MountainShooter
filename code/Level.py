...
class Level:
    def __init__(self, window: Surface, name: str, game_mode: str, player_score: list[int]):
        ...
        self.entity_list.extend(EntityFactory.get_entity(self.name + 'Bg'))  # Background
        player = EntityFactory.get_entity('Player1') # Player1
        player.score = player_score[0]
        self.entity_list.append(player)
        if game_mode in [MENU_OPTION[1], MENU_OPTION[2]]:
            player = EntityFactory.get_entity('Player2')  # Player1
            player.score = player_score[1]
            self.entity_list.append(player)
        pygame.time.set_timer(EVENT_ENEMY, SPAWN_TIME)
        pygame.time.set_timer(EVENT_TIMEOUT, TIMEOUT_STEP) # A cada 100ms checar condição de vitória
